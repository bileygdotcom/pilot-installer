...
class StackNameScreen(BaseScreen):
    """
    Экран задания имени стека.
    Создаёт папку /usr/ascon/<имя> и подпапки.
    """
    ...
    def validate(self, name):
        return re.match(r'^[a-zA-Z0-9-]+$', name) is not None and len(name) <= 8
    ...
    def create_stack_dirs(self):
        ...
        try:
            ...
            self.status = f"Ошибка создания папок: {e}"
            return False
    ...
    def draw_content(self):
        ...
        instruction = "Задайте имя стека (до 8 символов, латинские буквы, цифры, дефис)"
        x = max(0, (self.width - len(instruction)) // 2)
        safe_addstr(self.stdscr, 6, x, instruction)
        instruction2 = "Все данные стека будут храниться в /usr/ascon/<имя>"
        ...
        if self.validation_error:
            err_msg = "Недопустимые символы! Используйте a-z, 0-9, - (макс 8 символов)"
